...
void load_conf(char* file, ClusterInfo* cluster1, ClusterInfo* cluster2) {
    FILE* fp = fopen(file, "r");
    if (!fp) {
        printf("can not open file\n");
        exit(0);
    ...
    while (!feof(fp)) {
        ...
        if (line[0] != '!') {
            ...
            if (cnt == 1) {
                cluster1->ip[i] = malloc(20*sizeof(char));
                strcpy(cluster1->ip[i], temp1);
                cluster1->port[i] = atoi(temp2);
                cluster1->num++;
            }
            else {
                cluster2->ip[i] = malloc(20*sizeof(char));
                strcpy(cluster2->ip[i], temp1);
                cluster2->port[i] = atoi(temp2);
                cluster2->num++;
            }
            i++;
        }
          ...
          if(len>0){  //正常接收到回复
              if(strcmp(temp,"-1")!=0){
                ...
                if(type == 1){ //GET Course id
                    char cid[50]={'\0'},cname[100]={'\0'},capacity[20]={'\0'},selected[20]={'\0'};
                    ...
                    if(strncasecmp("200",line,3)==0){
                        char sid[50]={'\0'},sname[100]={'\0'},cid[50]={'\0'},cname[100]={'\0'};
                        len = read_line(buff+bias,line);
                        bias = bias + len + 1;
                        sscanf(line,"%s %s\n",sid,sname);
                        strcpy(result,"{\"id\":");
...
void onlysend(ClusterInfo* cluster, char* text){
    pthread_mutex_lock(&cfd_lock);
    for (int i = 0; i < cluster->num; i++) {
        if (!cluster->is_alive[i]) {//如果已断开，尝试重连
            Connect_to_Server(cluster, i);
...
int main(int argc, char *argv[])
{
  ...
  load_conf(file, &cluster1, &cluster2);
  ...
  while(1) {
        ...
        for(int i=0; i < ret; i++)
        {
            ...
            if(event[i].data.fd == server_sockfd){
                ...
                if(cnt == 0){
                    for (int i = 0; i < cluster1.num; i++) {
                        Connect_to_Server(&cluster1, i);
                    }
                    for (int i = 0; i < cluster2.num; i++) {
                        Connect_to_Server(&cluster2, i);
                    }
                    cnt++;
                }
                count++;
            ...
            else if(count>0)
            {
                ...
                cinfo->epfd = epfd;
                cinfo->cluster1 = &cluster1;
                cinfo->cluster2 = &cluster2;
