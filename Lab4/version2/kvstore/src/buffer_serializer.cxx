...
//space: op(4) + vector.size(8) + vector.size * string
void buffer_serializer::put_opstruct(const int32_t op, const std::vector<std::string> oprnd_){
    put_i32(op);
    uint64_t size = oprnd_.size();
    put_u64(size);
...
void buffer_serializer::get_opstruct(int32_t& op,std::vector<std::string>& oprnd_){
    op = get_i32();
    uint64_t size = get_u64();
    for(uint64_t i = 0; i < size; i++){
        std::string str = get_str();
        oprnd_.push_back(str);
    }
}
