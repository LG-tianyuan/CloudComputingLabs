...
type Raft struct {
	...
	//心跳信号
	heartChan chan bool
	//Online server number
	serverOn int
}

func NewRaft(id, ip, port string) *Raft {
	rf := new(Raft)
	rf.node = &NodeInfo{
		ID:   id,
		Port: port,
...
func (rf *Raft) sendHeartPacket() {
	...
	for {
		...
		rf.broadcast("Raft.HeartBeatResponse", rf.node, func(ply *RequestPly) {
			//fmt.Println("收到心跳检测", ply.Ok)
		})
		//最后一次心跳的时间
...
func (rf *Raft) becomeCandidate() bool {
	r := randRange(1, 50)
	r = r * 350
	fmt.Println("Rand Candidate sleep time:", r)
	//休眠随机时间后，再开始成为候选人
...
func (rf *Raft) election() bool {
	...
	var large ElectArgs
	large.ID = rf.node.ID
	large.Term = rf.currentTerm
	go rf.broadcast("Raft.Vote", large, func(ply *RequestPly) {
		//fmt.Println("after vote Election Candidate Term:", large.term)
		rf.voteChan <- ply.Ok
	})
	for {
		select {
		...
		case Ok := <-rf.voteChan:
			...
			if rf.vote >= nodeCount/2+1 && rf.currentLeader == "-1" {
				...
				go rf.broadcast("Raft.ConfirmationLeader", rf.node, func(ply *RequestPly) {
					fmt.Println("其他节点:是否同意[", rf.node.ID, "]为领导者", ply.Ok)
				})
				//有leader了，可以发送心跳包了
				rf.heartChan <- true
				return true
