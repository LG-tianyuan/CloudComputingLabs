...
void *http_handler(void *args)
{
  ...
  for(int i = 0; i < 5; i++){
    para->cluster1.ip[i] = (char*)malloc(20*sizeof(char));
    para->cluster2.ip[i] = (char*)malloc(20*sizeof(char));
  }
...
int main(int argc, char *argv[])
{
  ...
  if(argc < 9)
  {
    ...
    printf("Example: ./httpserver --ip 127.0.0.1 --port 8888 --threads 8 --config server.conf\n");
      return 1;
  }
  //解析参数
